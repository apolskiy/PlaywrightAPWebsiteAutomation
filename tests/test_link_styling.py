...
The stylesheet applies one hover colour to links across the profile header, the
...
coded colour, so a future re-theme that changes the hover colour everywhere
...
@allure.story("The copyright link shares the hover colour used in the tables")
...
def test_footer_owner_link_shares_the_table_hover_color(desktop_page: LandingPage) -> None:
    ...
    with allure.step("Capture the footer link's resting colour"):
        resting_color = landing_page.computed_color(footer_link)

    with allure.step("Capture the reference hover colour from a project table link"):
    ...
    with allure.step("Verify the footer link hovers to the same colour"):
        footer_hover_color = landing_page.hover_color(footer_link)
        assert footer_hover_color == table_hover_color, (
            "The copyright link does not share the site's hover colour: it "
    ...
    with allure.step("Verify hovering actually changes the colour"):
